...
public extension Work {
   ...
   func doFilter<I>(_ filterClosure: @escaping (I) -> Bool, on: DispatchQueue? = nil) -> Work<Out, Out>
   where Out == [I]
   {
      let work = Work<Out, Out>()
      work.savedResultClosure = savedResultClosure
      work.closure = { 
         guard let arr = $0.input else {
            $0.fail()
            return
         }

         let result = arr.filter(filterClosure)
...
public extension Work {
   ...
   @discardableResult
   func doCombineLatest<In1, In2, Out1, Out2>(_ work1: Work<In1, Out1>, _ work2: Work<In2, Out2>, on: DispatchQueue? = nil)
      -> Work<Void, (Out1, Out2)>
   {
      let newWork = Work<Void, (Out1, Out2)>(input: nil)
      newWork.savedResultClosure = savedResultClosure
   ...
   @discardableResult
   func doCombineLatest<In1, In2, In3, Out1, Out2, Out3>(_ work1: Work<In1, Out1>,
                                                         _ work2: Work<In2, Out2>,
                                                         _ work3: Work<In3, Out3>,
                                                         on: DispatchQueue? = nil)
      -> Work<Void, (Out1, Out2, Out3)>
